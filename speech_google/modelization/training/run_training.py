...
class SpecRunner1(SpecTensorboard):
    ...
    def __init__(self, dataset: Union[Dataset, ImageFolder], version: int, running_directory: str = "runs", means: List = [0.5], stds: List = [0.5]):
    ...
    def compile(self, drop_out_rate: float = 0.3, num_units1: int = 3000, num_units2: int = 1000, num_units3: int = 300, output_size: int = 7, learning_rate: float = 0.0001, batch_size: int = 5):
        ...
        self.model: nn.Module = SpecModel1(drop_out_rate, num_units1, num_units2, num_units3, output_size)
    ...
    def train(self, epochs: int = 50):
        ...
        for epoch in tqdm(range(self.epochs)):
            for inputs, labels in self.loader:
                ...
                if n_batches % 100 == 0:
                    self.writer.add_scalar('accuracy', accuracies*100 / (100*self.batch_size), global_step=n_batches)    
                    self.writer.add_scalar('losses', losses/100, global_step=n_batches)    
                    losses = 0
                    accuracies = 0
...
class SpecRunner2(SpecRunner1):
    ...
    def compile(self, drop_out_rate: float = 0.3, num_units1: int = 3000, num_units2: int = 1000, num_units3: int = 300, output_size: int = 7, learning_rate: float = 0.0001, batch_size: int = 5):
        ...
        Args:
            ...
            batch_size (int, optional): La taille d'un lot d'images. Defaults to 5.
        """
        super().compile(drop_out_rate, num_units1, num_units2, num_units3, output_size, learning_rate, batch_size)
